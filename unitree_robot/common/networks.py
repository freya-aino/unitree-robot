...
class NetworkBlock(nn.Module):
    ...
    def forward(self, x: T.Tensor) -> T.Tensor:
        ...
        return self.act(self.nn3(self.ln(self.act(self.nn2(x)))))
...
class BasicPolicyValueNetwork(nn.Module):
    def __init__(
        ...
        self.policy_network = nn.Sequential(
            NetworkBlock(input_size, hidden_size, nn.ReLU),
            *[
                NetworkBlock(hidden_size, hidden_size, nn.ReLU)
                for _ in range(network_layers)
            ],
            NetworkBlock(hidden_size, policy_output_size, nn.ReLU),
        )
